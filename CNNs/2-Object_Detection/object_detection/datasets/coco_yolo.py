...
class COCODetection(Dataset):
    ...
    def __init__(self, 
        ...
        if self.mode == "train" or self.mode == "val":
            self.mode = self.mode + "2017"
            self.imgs = os.listdir(os.path.join(PATH_IMAGES, self.mode))
            with open(PATH_ANNS_INFO + self.mode + ".json") as json_file:
                self.anns_data = json.load(json_file)
        else:
        ...
        if self.rect:
            s = [(cv2.imread(os.path.join(PATH_IMAGES, self.mode,f))).shape for f in self.imgs]
    ...
    def __getitem__(self, idx):
        ...
        ann_boxes = tuple(ann["bbox"] for ann in anns_img)  #[x, y, w, h]
        lbls = [ann["category_id"] for ann in anns_img]
        areas = [ann["area"] for ann in anns_img]
        iscrowd = [ann["iscrowd"] for ann in anns_img]
        ann_boxes_norm = normalize_bboxes(ann_boxes, (h0, w0))
        labels_ori = ann_boxes_norm
        for i in range(len(ann_boxes_norm)):
            labels_ori[i].insert(0, lbls[i])
...
def normalize_bboxes(boxes, shape):
    h0, w0 = shape
    if isinstance(boxes, tuple):
        norm_bboxes = []
        for i in range(len(boxes)):
            norm_bboxes.append(normalize_bboxes(boxes[i], (h0, w0)))
        return norm_bboxes
    else:
        x_max = boxes[0] + boxes[2]
        y_max = boxes[1] + boxes[3]
        xmin, ymin, xmax, ymax = albu.augmentations.bbox_utils.normalize_bbox((boxes[0], # x_min
                                                                               boxes[1], # y_min
                                                                               x_max, # x_max
                                                                               y_max), # y_max
                                                                               h0, 
